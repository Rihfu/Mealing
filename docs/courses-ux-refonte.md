...
- La section « Déjà pris » reste en `LineRow` (server). Vérifié en direct : drag « Poulet » → Fruits & légumes (déplacé + persisté) ; coche « Tomate » (pastille verte + atténuation → Déjà pris) ; aucune erreur console ; données de test nettoyées.
