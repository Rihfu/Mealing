...
- **Nutrition** : **à la demande** — le catalogue n'a aucune nutrition stockée (0/340), donc bouton « Récupérer la nutrition (USDA/OFF) » → `fetchAndStoreNutrition` (persiste `nutrient_value`, affiche). **Jamais l'IA** pour les valeurs (garde-fou n°3) ; en revanche l'IA **traduit le nom FR en terme de recherche EN** (`ai/translate-food.ts`) car USDA est anglophone (« Œufs » ne matchait rien → « egg »), puis prend le 1er résultat fournisseur **plausible**. **Garde-fou de plausibilité** (`isPlausibleNutrition` + `ai/validate-nutrition.ts`) : filet algo (rejette un macro en g > 100, ou somme macros > 110 — écarte ex. « Sel » à 1333 g de glucides) + juge IA indulgent (rejette l'incohérence manifeste par type d'aliment, ex. glucides dans du sel ; l'IA juge seulement, ne fournit aucune valeur). `getFoodNutrition` lit le stocké.
