...
- **Nutrition** : **à la demande** — le catalogue n'a aucune nutrition stockée (0/340), donc bouton « Récupérer la nutrition (USDA/OFF) » → `fetchAndStoreNutrition` (persiste `nutrient_value`, affiche). **Jamais l'IA** pour les valeurs (garde-fou n°3) ; en revanche l'IA **traduit le nom FR en terme de recherche EN** (`ai/translate-food.ts`) car USDA est anglophone (« Œufs » ne matchait rien → « egg »), puis prend le 1er résultat fournisseur **plausible** (examen **plafonné à 4 candidats**/recherche — frugalité quota USDA 1000 req/h). **Garde-fou de plausibilité** (`isPlausibleNutrition` + `ai/validate-nutrition.ts`) : filet algo (rejette un macro en g > 100, ou somme macros > 110 — écarte ex. « Sel » à 1333 g de glucides) + juge IA indulgent (rejette l'incohérence manifeste par type d'aliment, ex. glucides dans du sel ; l'IA juge seulement, ne fournit aucune valeur). **Cache partagé** : le résultat est persisté sur les tables **globales** `food`/`nutrient_value` (RLS select = tous authentifiés) → un aliment déjà recherché est resservi à **n'importe quel utilisateur** par `getFoodNutrition`, sans nouvel appel API.
...
- **Entrées** : produits cliquables dans les stats (Tes incontournables, À racheter bientôt), **nom d'article cliquable dans la liste de courses** (`shopping-list.tsx`, si l'article a un `foodId`), + **picker** de recherche (`product-picker.tsx` → `resolveCatalogFoodAction`, import paresseux d'un externe). Actions : `produit/[id]/actions.ts`. **Pas de migration.**
