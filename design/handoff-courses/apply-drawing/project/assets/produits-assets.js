/* ===================================================================
   ...
   (fill="currentColor" fill-opacity≈.10–.12) pour l'effet « croquis ».

   Usage :
     import { PRODUCTS, CATEGORY_ICONS, CATEGORIES, PROVENANCE,
              productSvg, findProduct, categoryIconSvg, findCategoryIcon }
       from './assets/produits-assets.js';
     el.innerHTML = productSvg('carotte', { size: 22 });
     const p = findProduct('saumon');         // -> { key, label, cat, svg }
     el.innerHTML = categoryIconSvg('panier'); // icône de rayon générique
   =================================================================== */
...
export const CATEGORIES = {
  ...
  epicerie:  { label: 'Épicerie & boulangerie',    tint: 'var(--color-sage-tint)',   ink: 'var(--color-sage-deep)' },
  ...
  plats:     { label: 'Plats & traiteur',          tint: 'var(--color-butter-tint)', ink: '#8a6d1f' },
...
/* ---- Icônes « catégorie / rayon » génériques ----
   À proposer quand l'utilisateur crée sa propre catégorie de courses.
   Même format de dessin que les produits : { key, label, svg }. */
export const CATEGORY_ICONS = [
  ...
  { key: 'boite',     label: 'Boîte',    svg: '<path d="M3 8l9-4 9 4-9 4Z" fill="currentColor" fill-opacity=".12"/><path d="M3 8v8l9 4 9-4V8M12 12v8"/>' },
...
const _byCatIco = Object.fromEntries(CATEGORY_ICONS.map((p) => [p.key, p]));

function _wrap(inner, opts) {
  opts = opts || {};
...
export function productSvg(key, opts) {
  return _wrap((_byKey[key] || FALLBACK).svg, opts);
}

export function findCategoryIcon(key) {
  return _byCatIco[key] || CATEGORY_ICONS[0];
}

export function categoryIconSvg(key, opts) {
  return _wrap((_byCatIco[key] || CATEGORY_ICONS[0]).svg, opts);
}
