...
const _byKey: Record<string, ProductDef> = Object.fromEntries(PRODUCTS.map((p) => [p.key, p]));
...
/** Résout une clé de picto à partir d'un slug catalogue (sans le préfixe `cat:`). */
export function resolveProduct(slug?: string | null): ProductDef {
  if (!slug) return FALLBACK;
  const k = slug.replace(/^cat:/, '');
  return _byKey[k] ?? _byKey[ALIASES[k] ?? ''] ?? FALLBACK;
}

export function productsByCategory(): Record<CategoryKey, ProductDef[]> {
  const out = {} as Record<CategoryKey, ProductDef[]>;
  (Object.keys(CATEGORIES) as CategoryKey[]).forEach((c) => (out[c] = []));
  PRODUCTS.forEach((p) => out[p.cat].push(p));
  return out;
}
