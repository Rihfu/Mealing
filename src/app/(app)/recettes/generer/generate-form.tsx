    ...
    <div className="flex flex-col gap-5">
      <form action={formAction} className="flex flex-col gap-2">
        <textarea
          ...
          className="rounded border border-line-strong px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900"
        />
        <button
          type="submit"
          disabled={pending}
          className="self-start rounded bg-green-strong px-4 py-2 text-sm text-white disabled:opacity-50 dark:bg-white dark:text-black"
        >
      ...
      {draft && (
        <div className="flex flex-col gap-3 rounded border border-line p-4 dark:border-gray-800">
          <div>
            <h2 className="text-lg font-semibold">{draft.name}</h2>
            {draft.description && <p className="text-sm text-ink-soft">{draft.description}</p>}
          ...
          {draft.tags.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {draft.tags.map((t) => (
                <span key={t} className="rounded bg-sage-tint px-1.5 py-0.5 text-xs dark:bg-gray-800">
                  {t}
                </span>
              ))}
            </div>
