  ...
  if (line.source === 'manual') {
    return (
      <li className="flex items-center gap-2 py-1">
        ...
        <span className={`text-sm ${line.checked ? 'text-gray-400 line-through' : ''}`}>
          {line.name} {qty && <span className="text-gray-500">· {qty}</span>}
        </span>
    ...
    <div className="flex flex-col gap-5">
      ...
      {(['recipe', 'recurring', 'manual'] as const).map((src) => (
        <section key={src}>
          ...
          <ul className="flex flex-col divide-y divide-gray-100 dark:divide-gray-800">
            ...
            {grouped[src].length === 0 && <li className="py-1 text-xs text-gray-400">Rien.</li>}
          </ul>
        </section>
      ))}
      ...
      <section className="rounded border border-gray-200 p-3 dark:border-gray-800">
        ...
        <form action={addManualAction} className="flex flex-wrap items-end gap-2 text-sm">
          <input name="label" placeholder="Article" required className="flex-1 rounded border border-gray-300 px-2 py-1.5 dark:border-gray-700 dark:bg-gray-900" />
          <input name="quantity" type="number" step="any" placeholder="qté" className="w-20 rounded border border-gray-300 px-2 py-1.5 dark:border-gray-700 dark:bg-gray-900" />
          <input name="unit" placeholder="unité" className="w-20 rounded border border-gray-300 px-2 py-1.5 dark:border-gray-700 dark:bg-gray-900" />
          <button className="rounded bg-black px-3 py-1.5 text-white dark:bg-white dark:text-black">Ajouter</button>
        </form>
      </section>

      <section className="rounded border border-gray-200 p-3 dark:border-gray-800">
        <h2 className="mb-2 text-sm font-semibold">Produits récurrents (café, lait…)</h2>
        <ul className="mb-2 flex flex-col divide-y divide-gray-100 text-sm dark:divide-gray-800">
          {(recurring ?? []).map((r) => {
          ...
          {(!recurring || recurring.length === 0) && (
            <li className="py-1 text-xs text-gray-400">Aucun produit récurrent.</li>
          )}
        </ul>
        <form action={addRecurringAction} className="flex flex-wrap items-end gap-2 text-sm">
          ...
          <input name="label" placeholder="ou libellé" className="rounded border border-gray-300 px-2 py-1.5 dark:border-gray-700 dark:bg-gray-900" />
          <button className="rounded bg-black px-3 py-1.5 text-white dark:bg-white dark:text-black">Ajouter</button>
        </form>
      </section>
    </div>
