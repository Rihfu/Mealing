...
export function StoreList({ groups }: { groups: StoreGroup[] }) {
  ...
  function toggle(it: StoreItem) {
    ...
    if (!it.checked && prices[it.key] == null && it.suggestedPrice != null) setPrice(it.key, String(it.suggestedPrice));
    startTransition(async () => {
      const fd = new FormData();
      fd.set('item_key', it.key);
      fd.set('checked', String(!it.checked));
      await toggleCheckAction(fd);
    });
  }

  const checked = groups.flatMap((g) => g.items).filter((i) => i.checked);
          ...
          <div key={g.key}>
            ...
            <div className="flex flex-col gap-2.5">
              {g.items.map((it) => (
                <div
                  key={it.key}
                  className={`overflow-hidden rounded-2xl border ${it.checked ? 'border-line bg-paper' : 'border-line bg-surface shadow-soft'}`}
                >
                  <button
                    type="button"
                    onClick={() => toggle(it)}
                    ...
                    style={{ minHeight: 64 }}
                  >
                    <BigCheck checked={it.checked} />
                    <span className={`flex-1 text-[17px] font-semibold ${it.checked ? 'text-ink-soft line-through' : ''}`}>{it.name}</span>
                    {it.qty && <span className={`text-sm text-ink-soft ${it.checked ? 'line-through' : ''}`}>{it.qty}</span>}
                  </button>
                  {it.checked && (
                    <div className="flex items-center gap-2 border-t border-line bg-surface px-4 py-2.5">
                      ...
                      <span className="ml-auto text-xs text-ink-soft">facultatif</span>
                    </div>
                  )}
                </div>
              ))}
