...
1. **Poursuivre l'optimisation de la section Liste de courses** (chantier principal — beaucoup de détails restent à ajuster ; voir `docs/courses-ux-refonte.md`).
   ...
   - ⏳ **Reste** : enrichir synonymes/catalogue à l'usage ; finitions diverses. (Périmètre Courses majeur entièrement couvert et testé en direct.) ⚠️ Constat 5 : la fusion stock sur **unités incompatibles** (ex. c. à soupe vs g) reste non sommée — à traiter avec la normalisation d'unités de la **section Stock**. ⚠️ À confirmer sur device : appui long pour glisser tuiles (#2) et rayons (#6).
