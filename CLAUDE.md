...
1. **Poursuivre l'optimisation de la section Liste de courses** (chantier principal — beaucoup de détails restent à ajuster ; voir `docs/courses-ux-refonte.md`).
   ...
   - 🔧 **À peaufiner — constats du test live de bout en bout (2026-06-18, foyer Maison/SAWADA)** : circuit complet rejoué (`docs/courses-ux-refonte.md` pourra accueillir le détail). Tout le périmètre est ✅ ; restent ces points :
     1. **« Ajouter les ingrédients manquants » ajoute des articles déjà en stock** (Œufs, Beurre apparus + marqués « déjà en stock »). Couverture stock (`recipeMissingIngredients`/`loadStockCoverage`, `core/shopping.ts`) trop permissive, ou volontaire (avec l'avertissement) ? **À trancher** — point le plus important.
     2. **Re-clic = doublons** : `addRecipeMissingToShoppingAction` ré-insère les mêmes lignes (dédup seulement à l'affichage par clé canonique, pas en base). Envisager un anti-doublon vs lignes déjà présentes.
     3. **Pas de « supprimer » sur les lignes liées au catalogue** : seules les lignes 100 % manuelles (`manualOnly`, texte libre) ont « supprimer ». Vérifier si c'est voulu (sinon, permettre de retirer une ligne `repas`/`essentiel`/catalogue de la liste courante).
     4. **`add-article.tsx` — course async** : cliquer « Ajouter à la liste » pendant que les suggestions USDA arrivent encore décale la liste et peut sélectionner une suggestion par erreur. Verrouiller la liste / ignorer les résultats tardifs pendant la soumission (ou figer `selected`).
     5. **`checkoutPurchasedToStock` + unités incompatibles** : à l'achat→stock, un article dont l'unité ne se convertit pas vers la ligne de stock existante semble n'incrémenter **rien** (Beurre 40 g et Sel inchangés ; seul Œufs pièce+pièce a fusionné). À vérifier — concerne surtout la **section Stock** (prochaine section).
     6. *(info, pas un bug)* `computeShoppingStats` : « courses/semaine » explose (98) si plusieurs relevés tombent le même jour (intervalle ~0 j). Envisager une borne min d'intervalle pour l'extrapolation.
   - ⏳ **Reste** : enrichir synonymes/catalogue à l'usage ; finitions diverses + constats ci-dessus. (Périmètre Courses majeur entièrement couvert et testé en direct.)
