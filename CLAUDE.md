...
- **Fiche produit** : `/courses/produit/[foodId]` (`produit/[id]/page.tsx`, `sections.tsx`, `actions.ts`). `computeProductStats` (`core/shopping-product.ts`) = évolution du prix + habitudes. Nutrition **à la demande** (`fetchAndStoreNutrition`/`getFoodNutrition` dans foods.ts — fournisseur, jamais l'IA ; le catalogue n'a pas de nutrition pré-stockée). Conservation **par lieu de stockage, normes FR** : pont `src/lib/food-conservation.ts` + table `conservation_guideline` (migration `0016`, catégorie × placard/frigo/congélateur, curée FR, indicative) via `getConservationForFood`. Conseils IA indicatifs à la demande (`src/lib/ai/product-tips.ts`). Entrées : produits cliquables dans les stats + `product-picker.tsx`. **Dernière migration en base = 0016.** Détail : `docs/courses-ux-refonte.md` (§ Fiche produit).
