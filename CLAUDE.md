...
- **Stock** : graphes d'**évolution du stock d'un aliment** + **gaspillage**. ✅ Le **journal d'événements stock** (`stock_event`, in/out/adjust/discard) est désormais EN PLACE (cf. « Refonte de la section Stock » ci-dessus) → les stats sont débloquées (reste à les coder ; peu de données historisées pour l'instant).
...
**Fichiers Stock** : `src/app/(app)/stock/` (`page.tsx` serveur, `actions.ts`, `add-stock.tsx`, `stock-list.tsx`, `stock-tools.tsx`, `locations.ts`, `locations-manager.tsx`, `meal-reconcile.tsx`) ; `core/{storage,stock,stock-events,conservation,consumption}.ts` ; `ai/product-conservation.ts`.
...
**RESTE Stock** : surbrillance au retour de fiche produit (`?viewed`, minor) ; outils Stock pour l'agent IA (après routage Gemini) ; stats gaspillage/évolution (le journal `stock_event` est prêt, peu de données) ; cache-first du Stock (PWA, fluidité) ; éventuellement le glisser-de-lieu (si demandé).
