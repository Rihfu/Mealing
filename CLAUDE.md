...
1. **Poursuivre l'optimisation de la section Liste de courses** (chantier principal — beaucoup de détails restent à ajuster ; voir `docs/courses-ux-refonte.md`).
   ...
   - ⏳ **Reste** : **chantier D — import USDA/OFF assisté IA** (un appel → `{ nom générique FR, rayon ∈ liste }`, nutrition inchangée) ; enrichir synonymes/catalogue à l'usage ; **UX-14 mode magasin mobile**.
